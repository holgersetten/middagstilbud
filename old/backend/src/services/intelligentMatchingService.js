...
const enhancedCategoryService = require('./enhancedCategoryService');
...
class IntelligentMatchingService {
    ...
    async findMatchingOffers(ingredient, offers) {
        ...
        console.log(`🔍 Søker etter "${ingredient}" med forbedret kategorisystem...`);

        // Bruk det nye forbedrede kategorisystemet
        const categoryResults = enhancedCategoryService.findOffersForIngredient(ingredient, offers);
        
        if (categoryResults.length > 0) {
            console.log(`✅ Fant ${categoryResults.length} tilbud via kategori-matching`);
            
            // Legg til score og sorter
            const scoredResults = categoryResults.map(offer => {
                const category = enhancedCategoryService.categorizeOffer(offer);
                return {
                    ...offer,
                    matchScore: category ? category.score + 5 : 5, // Bonus for kategori-match
                    matchReason: category ? `Kategori-match: ${category.categoryName}` : 'Kategori-match'
                };
            });
        ...
        // Fallback til gammelt system hvis ingen kategori-matches
        console.log(`⚠️ Ingen kategori-matches, bruker fallback-søk for "${ingredient}"`);
        return this.legacyFindMatchingOffers(ingredient, offers);
    }

    legacyFindMatchingOffers(ingredient, offers) {
