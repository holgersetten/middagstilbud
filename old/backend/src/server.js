...
const enhancedCategoryService = require('./services/enhancedCategoryService');
...
// GET /api/categories - Hent alle tilgjengelige kategorier
app.get('/api/categories', (req, res) => {
    try {
        const categories = enhancedCategoryService.getAllCategories();
        res.json(categories);
    } catch (error) {
        console.error('❌ Feil i categories endpoint:', error.message);
        res.status(500).json({ error: 'Kunne ikke hente kategorier' });
    }
});

// GET /api/offers/category/:categoryId - Hent tilbud for en spesifikk kategori  
app.get('/api/offers/category/:categoryId', (req, res) => {
    try {
        const { categoryId } = req.params;
        const allOffers = offerService.getAllOffers();
        const categoryOffers = enhancedCategoryService.getOffersByCategory(allOffers, categoryId);
        
        res.json({
            categoryId,
            categoryName: enhancedCategoryService.categories[categoryId]?.name || 'Ukjent kategori',
            offers: categoryOffers.slice(0, 50) // Begrenset til 50 for ytelse
        });
    } catch (error) {
        console.error('❌ Feil i category offers endpoint:', error.message);
        res.status(500).json({ error: 'Kunne ikke hente tilbud for kategori' });
    }
});
