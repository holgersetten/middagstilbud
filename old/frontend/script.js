// Vis alle middagsforslag ved oppstart
...
const state = {
  ...
  currentMeal: null // Store current meal information
};
...
function showTab(tabName) {
  ...
  // Load categories when showing kategorier tab
  if (tabName === 'kategorier') {
    setTimeout(() => loadCategories(), 100);
  }

  // Load all meal suggestions when showing middager tab
  if (tabName === 'middager') {
    // Always go back to all meal suggestions when clicking middager tab
...
async function loadProductImages() {
  ...
  for (const card of offerCards) {
    ...
    try {
      ...
      const url = `${API_BASE_URL}/api/product-image/${encodeURIComponent(cleanStoreName)}/${encodeURIComponent(cleanHeading)}`;
...
async function searchInMealNames(searchTerm) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/meals?search=${encodeURIComponent(searchTerm)}`);
...
function renderMatchingMeals(meals, searchTerm) {
  ...
  meals.forEach(meal => {
    const ingredients = formatIngredients(meal.ingredients).toLowerCase();
    const icon = getMealIcon(meal.name);
    html += `
      <div class="meal-suggestion" onclick="loadMealSuggestion('${ingredients}', '${meal.name}', ${meal.time}, ${meal.vegetarian}, ${meal.vegan || false})">
        <div class="meal-icon">${icon}</div>
        <h4>${meal.name}</h4>
        <p>${formatIngredients(meal.ingredients)}</p>
      </div>`;
  });
...
function loadMealSuggestion(ingredients, mealName, time, vegetarian, vegan) {
  // Hide search section when viewing specific meal
  toggleSearchSection(false);
  
  // Store meal information for display
  state.currentMeal = {
    name: mealName,
    time: time,
  ...
  // Search for offers with meal info display
  const ingredientsString = Array.from(state.selectedTags).join(', ');
  searchBestOffers(ingredientsString, true);
}

async function loadAllMealSuggestions() {
  // Show search section when viewing all meals
  toggleSearchSection(true);
  
  // Clear current meal state and reset tags
  state.currentMeal = null;
  state.selectedTags.clear();
  
  // Clear search input and update tags display
  const ingredientsInput = document.getElementById('ingredients-input');
  if (ingredientsInput) {
    ingredientsInput.value = '';
  }
...
function formatIngredients(ingredients) {
  // Convert ingredients array to readable string, handling both strings and objects with alternatives
  return ingredients.map(ingredient => {
    if (typeof ingredient === 'string') {
      return ingredient;
    } else if (ingredient.alternatives && Array.isArray(ingredient.alternatives)) {
      return ingredient.alternatives.join(' eller ');
    } else {
...
function toggleSearchSection(show = true) {
  const headerSection = document.querySelector('#middager-content .header-section');
  const searchSection = document.querySelector('#middager-content .search-section');
  
  if (headerSection) {
    headerSection.style.display = show ? 'block' : 'none';
  }
  if (searchSection) {
...
function toggleIngredientTag(ingredient) {
  if (state.selectedTags.has(ingredient)) {
    state.selectedTags.delete(ingredient);
  } else {
    state.selectedTags.add(ingredient);
  }
  
  // Update the search with new ingredient list
  const ingredientsString = Array.from(state.selectedTags).join(', ');
  state.currentMeal.ingredients = ingredientsString;
  
  // Re-search with updated ingredients
  searchBestOffers(ingredientsString, true);
}
...
async function searchBestOffers(ingredients, showMealInfo = false) {
  ...
  try {
    ...
    if (showMealInfo && state.currentMeal) {
      const meal = state.currentMeal;
      let html = '<div class="meal-info-header">';
      html += '<div class="meal-details">';
    html += `<div class="meal-title">`;
    html += `<div class="meal-title-left">`;
    html += `<span class="meal-icon-large">${getMealIcon(meal.name)}</span>`;
    html += `<h2>${meal.name}</h2>`;
    html += `</div>`;
    html += `<div class="meal-time">⏱️ ${meal.time} min</div>`;
    html += '</div>';
    
    html += '<div class="meal-meta">';      if (meal.vegetarian) html += '<div class="meal-badge vegetarian">🌱 Vegetar</div>';
      if (meal.vegan) html += '<div class="meal-badge vegan">🌿 Vegan</div>';
      
      html += '</div>';
      
      html += `<div class="meal-ingredients">`;
      html += `<strong>Ingredienser:</strong> ${meal.ingredients}`;
      html += '</div>';
      
      html += '<button class="back-button" onclick="loadAllMealSuggestions()">← Tilbake til alle middager</button>';
      
      html += '</div>';
      html += '</div>';
      html += '<div class="loading">Søker etter tilbud for ingrediensene...</div>';
    ...
    const response = await fetch(`${API_BASE_URL}/api/best-offers?ingredients=${encodeURIComponent(ingredients)}`);
    const data = await response.json();
    ...
    console.log('🎯 About to call renderBestOffers with showMealInfo:', showMealInfo);
    renderBestOffers(showMealInfo);
...
function calculateMealPrice(offersData) {
  ...
  offersData.forEach(group => {
    if (group.offers && group.offers.length > 0) {
      // Find the cheapest offer for this ingredient
  ...
  return ingredientsWithPrices > 0 ? Math.round(totalPrice) : null;
}

function renderBestOffers(showMealInfo = false) {
  const content = document.getElementById('content');
  const placeholderMeals = document.getElementById('placeholder-meals');
  ...
  if (!state.offersRaw || state.offersRaw.length === 0) { 
    console.log('❌ No offersRaw data or empty array');
  ...
  // Check if any group has offers
  const hasAnyOffers = state.offersRaw.some(group => {
    console.log('🔍 Checking group:', group.ingredient, 'offers:', group.offers?.length || 0);
    return group.offers && group.offers.length > 0;
  });
  
  console.log('✅ hasAnyOffers:', hasAnyOffers);
  
  if (!hasAnyOffers) {
    console.log('❌ No groups have offers, showing ingredient suggestions');
    // Show ingredient suggestions if no groups have any offers
    renderIngredientSuggestions();
    return;
  }
  ...
  // Add meal info header if this is from a meal suggestion
  if (showMealInfo && state.currentMeal) {
    const meal = state.currentMeal;
    const mealPrice = calculateMealPrice(state.offersRaw);
    
    html += '<div class="meal-info-header">';
    html += '<div class="meal-details">';
    html += `<div class="meal-title">`;
    html += `<div class="meal-title-left">`;
    html += `<span class="meal-icon-large">${getMealIcon(meal.name)}</span>`;
    ...
    if (mealPrice) {
      html += `<div class="meal-price">💰 ca. ${mealPrice} kr</div>`;
    }
    
    if (meal.vegetarian) html += '<div class="meal-badge vegetarian">🌱 Vegetar</div>';
    if (meal.vegan) html += '<div class="meal-badge vegan">🌿 Vegan</div>';
    
    html += '</div>';
    
    html += '<button class="back-button" onclick="loadAllMealSuggestions()">←</button>';
    
    // Add ingredient tags in a compact way
    html += '<div class="meal-ingredient-tags">';
    const ingredientList = meal.ingredients.split(',').map(i => i.trim());
    ingredientList.forEach(ingredient => {
      html += `<span class="ingredient-tag small clickable" onclick="toggleIngredientTag('${ingredient.toLowerCase()}')">${ingredient}</span>`;
    });
    // Add a + button to allow adding more ingredients
    html += `<span class="ingredient-tag small add-tag" onclick="expandAddTag(this)" id="add-tag-btn">+</span>`;
    html += '</div>';
  ...
  state.offersRaw.forEach(group => {
    ...
    if (group.offers.length === 0) {
      // Show empty state for ingredient with no offers
      html += `<div class="ingredient-section">
        <div class="ingredient-header">
          <h3>${group.ingredient.charAt(0).toUpperCase() + group.ingredient.slice(1)}</h3>
          <span class="count-badge empty">Ingen tilbud</span>
        </div>
        <div class="empty-ingredient">
          <p>Ingen tilbud funnet for ${group.ingredient}</p>
        </div>
      </div>`;
      return;
    }
...
function expandAddTag(element) {
  // Check if already expanded
  if (element.classList.contains('expanded')) {
    // Collapse and restore to plus
  ...
  element.innerHTML = '<input type="text" placeholder="Legg til ingrediens..." maxlength="30" onkeydown="handleAddTagInput(event)" onblur="collapseAddTag(this)" autofocus>';
...
function handleAddTagInput(event) {
  const input = event.target;
  const addTagElement = input.parentElement;
  
  if (event.key === 'Enter') {
    const ingredient = input.value.trim();
    if (ingredient) {
      // Add the ingredient to selected tags
      state.selectedTags.add(ingredient.toLowerCase());
      
      // Update the current meal ingredients
      const ingredientsString = Array.from(state.selectedTags).join(', ');
      state.currentMeal.ingredients = ingredientsString;
    ...
    // Collapse the add tag
    collapseAddTag(input);
  } else if (event.key === 'Escape') {
    collapseAddTag(input);
  }
}

function collapseAddTag(input) {
  const addTagElement = input.parentElement;
  addTagElement.classList.remove('expanded');
...
async function loadCategories() {
  ...
  try {
    ...
    document.getElementById('categories-grid').innerHTML = 
      '<div class="empty"><h3>Feil ved lasting</h3><p>Kunne ikke laste kategorier.</p></div>';
  }
}
...
function renderCategoriesGrid(categories) {
  ...
  categories.forEach(category => {
    ...
    html += `
      <div class="category-card" onclick="loadCategoryOffers('${category.id}', '${category.name}')">
        <div class="category-icon">${icon}</div>
        <h3>${category.name}</h3>
        <p>${category.keywordCount} nøkkelord</p>
      </div>
    `;
  });
...
async function loadCategoryOffers(categoryId, categoryName) {
  console.log(`📂 Loading offers for category: ${categoryId}`);
  
  try {
    document.getElementById('category-offers-content').innerHTML = 
      '<div class="loading">Laster tilbud...</div>';
      
    const response = await fetch(`${API_BASE_URL}/api/offers/category/${categoryId}`);
    const data = await response.json();
    
    renderCategoryOffers(data);
  } catch (error) {
    console.error('❌ Error loading category offers:', error);
    document.getElementById('category-offers-content').innerHTML = 
      '<div class="empty"><h3>Feil ved lasting</h3><p>Kunne ikke laste tilbud for kategorien.</p></div>';
  }
}

function renderCategoryOffers(data) {
  const content = document.getElementById('category-offers-content');
  
  let html = `
    <div class="category-offers-section">
      <div class="category-offers-header">
        <h3>${data.categoryName}</h3>
        <button class="back-to-categories-btn" onclick="backToCategories()">← Tilbake til kategorier</button>
      </div>
      <div class="offers-grid fixed-size">
  `;
  
  if (data.offers.length === 0) {
    html += '<div class="empty"><p>Ingen tilbud funnet for denne kategorien.</p></div>';
  } else {
